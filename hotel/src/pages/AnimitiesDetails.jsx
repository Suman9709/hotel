...
const AnimitiesDetails = () => {
    ...
    if (!amenity) return <h1 className="text-center mt-24 text-xl font-semibold">Amenity Not Found</h1>

    return (
        <div className='mt-24 px-6 sm:px-10 lg:px-16'>
            {/* Title */}
            <h1 className='text-xl sm:text-4xl font-bold text-center text-gray-800 mb-8' style={{ fontFamily: "Times New Roman, serif" }}>
                {amenity.title}
            </h1>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-4'>
                {amenity.images.map((image, index) => (
                    <div key={index} className='overflow-hidden rounded-xl shadow-lg hover:scale-105 transform transition duration-300'>
                        <img
                            src={image}
                            alt={`${amenity.title} ${index + 1}`}
